...
public abstract class GroupRecyclerAdapter<Parent, Child> extends BaseRecyclerAdapter<Child> {
     ...
     * @return 获取所在分组
     */
    public Parent getGroupByChildPosition(int position) {
        int count = 0;
        if (position <= count)
            return null;
        int i = 0;
     ...
     * @return 获取所在位置
     */
    public int getPositionByGroup(String group) {
        int count = 0;
        if (ToolsHelper.isNull(group))
            return 0;
        for (int i = 0; i < getGroupCount(); i++) {
            if (group.equals(mGroupTitles.get(i))) {
                return count;
            }
            if (i == 0) {
