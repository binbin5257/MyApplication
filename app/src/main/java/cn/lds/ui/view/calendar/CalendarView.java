...
public class CalendarView extends FrameLayout {
     ...
     *         日期选中事件
     */
    public void setOnDateSelectedListener(CalendarView.OnDateSelectedListener listener) {
    ...
    public void selectDate(Calendar date) {
        if (mDelegate.mDateSelectedListener != null) {
            ...
            if (!Util.isCalendarInRange(date, mDelegate)) {
                return;
            }
            mDelegate.mSelectedCalendar = date;
            if (View.VISIBLE == mWeekBar.getVisibility()) {
                mWeekPager.updateSelected(date);
            } else {
                mMonthPager.updateSelected();
            }
        }
